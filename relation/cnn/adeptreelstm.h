#ifndef CNN_ATTENTION_DEPTREELSTM_H_
#define CNN_ATTENTION_DEPTREELSTM_H_
...
struct AttentionDepTreeLSTMBuilder : public RNNBuilder {
  AttentionDepTreeLSTMBuilder() = default;
  explicit AttentionDepTreeLSTMBuilder(unsigned N, //Max branching factor
                              unsigned layers,
                              unsigned input_dim,
                              unsigned hidden_dim,
                              Model* model,
  ...
  Expression add_input(std::vector<std::pair<int, int> > children, const Expression& x);
protected:
