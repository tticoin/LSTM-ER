...
enum { X2I, C2I, BI, X2F, C2F, BF, X2O, C2O, BO, X2C, BC, C2A, BA, UA };
...
AttentionDepTreeLSTMBuilder::AttentionDepTreeLSTMBuilder(unsigned N,
                                       unsigned layers,
                                       unsigned input_dim,
                                       unsigned hidden_dim,
                                       Model* model,
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    // x attention
    Parameters* p_c2a = model->add_parameters({hidden_dim, hidden_dim});
    Parameters* p_ba = model->add_parameters({layer_input_dim});
    Parameters* p_ua = model->add_parameters({layer_input_dim});

    vector<Parameters*> ps = {p_x2i, p_c2i, p_bi, p_x2f, p_c2f, p_bf, p_x2o, p_c2o, p_bo, p_x2c, p_bc, p_c2a, p_ba, p_ua};
...
void AttentionDepTreeLSTMBuilder::new_graph_impl(ComputationGraph& cg){
  param_vars.clear();
  ...
  for (unsigned i = 0; i < layers; ++i){
    ...
    //attention
    Expression i_c2a = parameter(cg, p[C2A]);
    Expression i_ba = parameter(cg, p[BA]);
    Expression i_ua = parameter(cg, p[UA]);

    
    vector<Expression> vars = {i_x2i, i_c2i, i_bi, i_x2f, i_c2f, i_bf, i_x2o, i_c2o, i_bo, i_x2c, i_bc, i_c2a, i_ba, i_ua};
...
// layout: 0..layers = c
//         layers+1..2*layers = h
void AttentionDepTreeLSTMBuilder::start_new_sequence_impl(const vector<Expression>& hinit) {
  h.clear();
  c.clear();
  if (hinit.size() > 0) {
...
// n-ary
Expression AttentionDepTreeLSTMBuilder::add_input(vector<pair<int, int> > children, const Expression& x) {
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    if (children.size() == 0) {
      ...
      if(children.size() > 1){
        vector<Expression> attentions;
        for (unsigned j = 0; j < children.size(); ++j) {
          vector<Expression> xs = {vars[BA], vars[C2A], i_h_children[j]};
          attentions.push_back(dot_product(tanh(affine_transform(xs)), vars[UA]));
        }
        Expression aw = softmax(concatenate(attentions));
        for (unsigned j = 0; j < children.size(); ++j) {
          i_h_children[j] = i_h_children[j] * select_rows(aw, {j});
        }        
      }      
    }
    // input
...
Expression AttentionDepTreeLSTMBuilder::add_input_impl(int prev, const Expression& x) {
  assert (false);
  return x;
}

void AttentionDepTreeLSTMBuilder::copy(const RNNBuilder & rnn) {
  const AttentionDepTreeLSTMBuilder & rnn_treelstm = (const AttentionDepTreeLSTMBuilder&)rnn;
