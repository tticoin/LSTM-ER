...
DepTreeLSTMBuilder::DepTreeLSTMBuilder(unsigned N,
                                       unsigned layers,
                                       unsigned input_dim,
                                       unsigned hidden_dim,
                                       Model* model,
...
// layout: 0..layers = c
//         layers+1..2*layers = h
void DepTreeLSTMBuilder::start_new_sequence_impl(const vector<Expression>& hinit) {
  h.clear();
  c.clear();
  if (hinit.size() > 0) {
...
// n-ary
Expression DepTreeLSTMBuilder::add_input(vector<pair<int, int> > children, const Expression& x) {
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    if (children.size() == 0) {
      ...
      for (pair<int, int> child : children) {
        i_h_children.push_back(h[child.second][i]);
        i_c_children.push_back(c[child.second][i]);
      }
    }
    ...
    if (has_prev_state) {
      ...
      for (unsigned j = 0; j < children.size(); ++j) {
        unsigned ej = (children[j].first < (int)N) ? children[j].first : N - 1;
    ...
    for (unsigned k = 0; k < children.size(); ++k) {
      unsigned ek = (children[k].first < (int)N) ? children[k].first : N - 1;
      Expression i_aft;
      if (has_prev_state) {
...
//     }
//     if (has_prev_state) {
//       vector<Expression> ict(children.size());
//       for (unsigned j = 0; j < children.size(); ++j) {
//         unsigned ej = (children[j].first < (int)N) ? children[j].first : N - 1;
//         // input
//         Expression i_it;
//         {
//           vector<Expression> xs = {vars[BI], vars[X2I], in[j]};
//           xs.reserve(5);
//           xs.push_back(lookup(cg, lparams[i][H2I], ej));
//           //xs.push_back(lparam_vars[i][j * (N + 3) + 0]);
//           xs.push_back(i_h_children[j]);
//           assert (xs.size() == 5);
//           Expression i_ait = affine_transform(xs);
//           i_it = logistic(i_ait);
//         }
...
//           unsigned ek = (children[k].first < (int)N) ? children[k].first : N - 1;
//           Expression i_aft;
//           vector<Expression> xs = {vars[BF], vars[X2F], in[j]};
//           xs.reserve(5);
//           xs.push_back(lookup(cg, lparams[i][H2F], ej * N + ek));
//           //xs.push_back(lparam_vars[i][j * (N + 3) + 3 + k]);
...
//         Expression i_wt;
//         {
//           vector<Expression> xs = {vars[BC], vars[X2C], in[j]};
//           // This is the one and only place that should *not* condition on i_c_children
//           // This should condition only on x (a.k.a. in), the bias (vars[BC]) and i_h_children
//           xs.reserve(5);
//           xs.push_back(lookup(cg, lparams[i][H2C], ej));
...
//           xs.push_back(i_h_children[j]);
//           assert (xs.size() == 5);
//           Expression i_awt = affine_transform(xs);
//           i_wt = tanh(i_awt);
//         }
...
//           vector<Expression> i_crts(children.size());
//           for (unsigned k = 0; k < children.size(); ++k) {
//             i_crts[k] = cwise_multiply(i_ft[k], i_c_children[k]);
//           }
//           Expression i_crt = sum(i_crts);
//           ict[j] = i_crt + i_nwt;
//         }
...
//         Expression i_aot;
//         {
//           vector<Expression> xs = {vars[BO], vars[X2O], in[j]};
//           xs.reserve(2 + 3);
//           xs.push_back(lookup(cg, lparams[i][H2O], ej));
//           //xs.push_back(lparam_vars[i][j * (N + 3) + 1]);
//           xs.push_back(i_h_children[j]);
...
//         // Compute new h value
//         Expression ph_t = tanh(ict[j]);
//         in[j] = cwise_multiply(i_ot, ph_t);
//       }
//       ct[i] = sum(ict);
//       ht[i] = sum(in);
//     }else{
//       // input
//       Expression i_ait = affine_transform({vars[BI], vars[X2I], in[0]});
//       Expression i_it = logistic(i_ait);

//       // forget
//       // vector<Expression> i_ft;
//       // Expression i_aft = affine_transform({vars[BF], vars[X2F], in[0]});
//       // i_ft.push_back(logistic(i_aft));

//       // write memory cell
//       Expression i_awt = affine_transform({vars[BC], vars[X2C], in[0]});
//       Expression i_wt = tanh(i_awt);
...
//       // output
//       Expression i_aot = affine_transform({vars[BO], vars[X2O], in[0]});
//       Expression i_ot = logistic(i_aot);

//       // Compute new h value
//       Expression ph_t = tanh(ct[i]);
//       in[i] = ht[i] = cwise_multiply(i_ot, ph_t);
//     }
//   }
//   return ht.back();
// }
...
//         vector<Expression> xs = {vars[BI], vars[X2I], in};
//         xs.reserve(5);
//         xs.push_back(lookup(cg, lparams[i][H2I], ei));
//         xs.push_back(h);
//         assert (xs.size() == 5);
//         Expression i_ait = affine_transform(xs);
//         i_it = logistic(i_ait);
...
//         vector<Expression> xs = {vars[BF], vars[X2F], in};
//         xs.reserve(5);
//         xs.push_back(lookup(cg, lparams[i][H2F], ej));
//         xs.push_back(i_h_children[j]);
//         assert (xs.size() == 5);
//         i_aft = affine_transform(xs);
//         i_ft.push_back(logistic(i_aft));
...
//         // This should condition only on x (a.k.a. in), the bias (vars[BC]) and i_h_children
//         xs.reserve(5);
//         xs.push_back(lookup(cg, lparams[i][H2C], ei));
//         xs.push_back(h);
//         assert (xs.size() == 5);
//         Expression i_awt = affine_transform(xs);
//         i_wt = tanh(i_awt);
...
//         vector<Expression> xs = {vars[BO], vars[X2O], in};
//         xs.reserve(2 + 3);
//         xs.push_back(lookup(cg, lparams[i][H2O], ei));
//         xs.push_back(h);
//         assert (xs.size() == 2 + 3);
//         i_aot = affine_transform(xs);
//       }
...
//     }else{
//       // input
//       Expression i_ait = affine_transform({vars[BI], vars[X2I], in});
//       Expression i_it = logistic(i_ait);

//       // forget
//       // vector<Expression> i_ft;
//       // Expression i_aft = affine_transform({vars[BF], vars[X2F], in});
//       // i_ft.push_back(logistic(i_aft));

//       // write memory cell
//       Expression i_awt = affine_transform({vars[BC], vars[X2C], in});
//       Expression i_wt = tanh(i_awt);
...
//       // output
//       Expression i_aot = affine_transform({vars[BO], vars[X2O], in});
//       Expression i_ot = logistic(i_aot);
...
Expression DepTreeLSTMBuilder::add_input_impl(int prev, const Expression& x) {
  assert (false);
  return x;
}

void DepTreeLSTMBuilder::copy(const RNNBuilder & rnn) {
  const DepTreeLSTMBuilder & rnn_treelstm = (const DepTreeLSTMBuilder&)rnn;
